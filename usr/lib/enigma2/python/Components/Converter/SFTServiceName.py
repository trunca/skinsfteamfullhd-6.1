...
class SFTServiceName(Converter, object):
    NAME = 0
    NUMBER = 1
    BOUQUET = 2
    PROVIDER = 3
    ...
    def getIPTVProvider(self, refstr):
        ...
        if 'udp/239.0.0.' in refstr:
            return 'Trinity'
        if 'udp/239.100.' in refstr or 'udp/233.252.8.' in refstr or 'udp/225.225.225.' in refstr or 'udp/225.1.' in refstr:
            return 'Volia TV'
        ...
        if 'tv.lifelink.ru' in refstr:
            return 'tv.lifelink.ru'
        if 'sat-elit.net' in refstr:
